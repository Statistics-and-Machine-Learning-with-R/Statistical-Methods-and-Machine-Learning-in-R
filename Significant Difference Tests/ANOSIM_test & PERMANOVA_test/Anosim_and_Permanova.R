"
  ...
  which will be asked on running the code in the CONSOLE screen. In this case select:
    ...
    b- Select the separator of the inserted file.
    c- Then, select Groupings_file_Anosim&Permanova.CSV, when you are asked to select the Groupings file
    d- Select whether you want to perform anosim or Permanova 
...
#Set the row and column headers for the R and p-value matrix
...
for (i in 1:nrow(uniqueGroups)){
  for (j in 1:nrow(uniqueGroups)){
    ...
    #Making Empty classification matrix
