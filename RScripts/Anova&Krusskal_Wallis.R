#-------------------------------------------------------------------------------------------------------------
#                                          Anova and Krusskal_Wallis TEST
#-------------------------------------------------------------------------------------------------------------
...
# Package for visualization and some manupulation
...
# or
outputname = "/Users/mac/Documents/12CP Project/ESULTS_ANOVA.csv"


#------------------------------------------------
# STEP 0: Gets the matrix and define groups
#------------------------------------------------
# Start Filechoose
...
#numb_of_groups = readline(prompt = "Enter Number of desired groups: ")
Group_start <- list()
Group_end <- list()
...
groups <- rep(1:3,index_for_new_groups)
...
shapiro_Wvalue          <-  (rep(0, nrow(matrix))) # + 19
shapiro_Pvalue          <-  (rep(0, nrow(matrix))) # + 20
...
for (i in 1:ncol(matrix)) { 
  Resulting_Matrix[i,ncol(matrix) + 14]   <- shapiro.test(as.numeric(matrix[,i]))$statistic # w-value
  Resulting_Matrix[i,ncol(matrix) + 15]   <- shapiro.test(as.numeric(matrix[,i]))$p.value   #p-value
}
...
ggplot()+
  ...
  geom_abline(aes(intercept=0,slope=1),color='blue')+xlab('Average of Group1')+ylab('Average of Group2')+labs(color='P-adj.Value')+ 
...
ggplot()+
  ...
  geom_abline(aes(intercept=0,slope=1),color='blue')+xlab('Average of Group1')+ylab('Average of Group3')+labs(color='P-adj.Value')+
...
ggplot()+
  ...
  geom_abline(aes(intercept=0,slope=1),color='blue')+xlab('Average of Group2')+ylab('Average of Group3')+labs(color='P-adj.Value')+
